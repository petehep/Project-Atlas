from PySide6.QtWidgets import (QMainWindow, QLabel, QVBoxLayout, QHBoxLayout, 
                    ...
                    QHeaderView, QMessageBox, QStackedWidget, QComboBox)
...
class OperatorWindow(QMainWindow):
    ...
    def _setup_schedule_tab(self):
        ...
        self.edit_heat_no = QLineEdit(); self.edit_dir = QLineEdit("LEFT")
        ...
        self.save_btn = QPushButton("ADD NEW HEAT"); self.save_btn.setStyleSheet(AtlasTheme.BTN_MASTER_ARM); self.save_btn.setFixedHeight(60)
        self.save_btn.clicked.connect(self._save_to_db); btn_layout.addWidget(self.save_btn)
        self.clear_btn = QPushButton("NEW / CLEAR"); self.clear_btn.setStyleSheet("background: #444; color: white; border-radius: 5px; font-weight: bold;"); self.clear_btn.setFixedHeight(60)
        self.clear_btn.clicked.connect(self._prep_new_heat); btn_layout.addWidget(self.clear_btn)
        self.delete_btn = QPushButton("DELETE"); self.delete_btn.setStyleSheet("background: #700; color: white; border-radius: 5px; font-weight: bold;"); self.delete_btn.setFixedHeight(60); self.delete_btn.setVisible(False)
        self.delete_btn.clicked.connect(self._delete_selected); btn_layout.addWidget(self.delete_btn)
    ...
    def _save_settings(self):
        ...
        # Extract width/height from combo
        selection = self.combo_disp_size.currentText()
        w, h = (128, 64) if "128" in selection else (64, 64)
    ...
    def _on_row_selected(self, row, column):
        ...
        self.edit_open.setTime(t_open); self.edit_close.setTime(t_close)
        self.save_btn.setText("UPDATE SELECTED HEAT"); self.delete_btn.setVisible(True)

    def _prep_new_heat(self):
        self._selected_heat_id = None; self.save_btn.setText("ADD NEW HEAT"); self.delete_btn.setVisible(False)
        heats = self.engine.db.get_todays_schedule(); next_no = len(heats) + 1
        self.edit_heat_no.setText(str(next_no))
        now = QDateTime.currentDateTime().time()
        self.edit_open.setTime(now.addSecs(300)); self.edit_close.setTime(now.addSecs(600)); self.edit_end.setTime(now.addSecs(3600))

    def _save_to_db(self):
        today = QDateTime.currentDateTime().date()
        def to_unix(qtime): return QDateTime(today, qtime.time()).toSecsSinceEpoch()
        t_open = to_unix(self.edit_open); t_close = to_unix(self.edit_close)
        if t_close <= t_open:
            QMessageBox.critical(self, "Validation Error", "Track Close must be after Track Open!"); return
        config = HeatConfig(id=self._selected_heat_id, heat_number=self.edit_heat_no.text(), thermalling_dir=self.edit_dir.text(), track_open=t_open, track_close=t_close, heat_end=to_unix(self.edit_end))
        self.engine.db.save_heat(config); self._update_schedule_table(); self._prep_new_heat()

    def _delete_selected(self):
        if self._selected_heat_id:
            reply = QMessageBox.question(self, 'Confirm', 'Delete this heat?', QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes: self.engine.db.delete_heat(self._selected_heat_id); self._update_schedule_table(); self._prep_new_heat()

    def _update_schedule_table(self):
        heats = self.engine.db.get_todays_schedule(); self.schedule_table.setRowCount(len(heats))
        for row, h in enumerate(heats):
            ...
            self.schedule_table.setItem(row, 3, QTableWidgetItem(t_open)); self.schedule_table.setItem(row, 4, QTableWidgetItem(t_close))
    ...
    def refresh(self, model):
        if model.is_armed: msg = f"H{model.heat_number} | {model.thermalling_dir}\n{model.primary_timer}"
        else: msg = "WAITING"
